...
def main():
    ...
    for _p in range(10):
        if _esc[_p].split(' ')[1] == '1': # 震度１なら黒く表示
            _c = colorama.Fore.WHITE + colorama.Back.BLACK
        if _esc[_p].split(' ')[1] == '2': # 震度２なら青く表示
            _c = colorama.Fore.WHITE + colorama.Back.BLUE
        if _esc[_p].split(' ')[1] == '3': # 震度３なら緑に表示
            _c = colorama.Fore.BLACK + colorama.Back.GREEN
        if _esc[_p].split(' ')[1] == '4': # 震度４なら黄色く表示
            _c = colorama.Fore.BLACK + colorama.Back.YELLOW
        if _esc[_p].split(' ')[1] == '5弱': # 震度５弱なら赤く表示
            _c = colorama.Fore.WHITE + colorama.Back.RED
        if _esc[_p].split(' ')[1] == '5強': # 震度５強なら赤く表示
            _c = colorama.Fore.WHITE + colorama.Back.RED
        if _esc[_p].split(' ')[1] == '6弱': # 震度６弱なら赤く表示
            _c = colorama.Fore.WHITE + colorama.Back.RED
        if _esc[_p].split(' ')[1] == '6強': # 震度６強なら赤く表示
            _c = colorama.Fore.WHITE + colorama.Back.RED
        if _esc[_p].split(' ')[1] == '7': # 震度７なら紫色に表示
            _c = colorama.Fore.WHITE + colorama.Back.MAGENTA
        if _esc[_p].split(' ')[1] == '---': # 震度不明なら黒く表示
